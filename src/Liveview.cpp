...
// 前缀常量
// ==================================================================
const char Liveview_::_PREFIX[] = "LV:";
...
Liveview_ &Liveview = Liveview_::getInstance();
...
void Liveview_::tick() {
  if (_interval == 0 || _callback == nullptr || _leds == nullptr)
    return;
  if ((millis() - _lastSample) < _interval)
    return;
  ...
  memcpy(ptr, _PREFIX, _PREFIX_LEN);
  ptr += _PREFIX_LEN;
  ...
  _bufLen = ptr - (uint8_t *)_buf;

  // --- CRC 判重 ---
  uint32_t crc = _crc32((const byte *)_buf, _bufLen);
  if (crc != _sentChecksum) {
    _pendingChecksum = crc;
    _dirty = true; // 标记有新帧等待发送
  }
}
...
void Liveview_::flush() {
  ...
  _dirty = false;
  _sentChecksum = _pendingChecksum;
  _callback((const char *)_buf, _bufLen);
}
...
uint32_t Liveview_::_crc32(const byte *data, size_t length) {
  ...
  for (size_t i = 0; i < length; i++) {
    ...
    crc = (crc >> 8) ^ pgm_read_dword(&crc32_table[idx]);
  }
  return ~crc;
}
