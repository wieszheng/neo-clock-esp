...
void ServerManager_::handleWebSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
{
    ...
    case WStype_CONNECTED:
    {
        ...
        // ==================== icon upload (.anim) ====================
        // 协议:
        // 1) TEXT: {type:'uploadIcon', requestId, phase:'start', filename, totalBytes, chunkSize}
        // 2) BIN : raw bytes, continuous
        // 3) TEXT: {type:'uploadIcon', requestId, phase:'finish'}
        // 返回: ack/error (带 requestId)
    ...
    case WStype_TEXT:
    {
        ...
        if (type == "uploadIcon")
        {
            String phase = doc.containsKey("phase") ? doc["phase"].as<String>() : String("");

            if (phase == "start")
            {
                String filename = doc.containsKey("filename") ? doc["filename"].as<String>() : String("");
                uploadTotal = doc.containsKey("totalBytes") ? (size_t)doc["totalBytes"].as<uint32_t>() : 0;
            ...
            if (phase == "finish")
            {
                ...
                bool ok = (uploadTotal == 0) ? (uploadReceived > 0) : (uploadReceived == uploadTotal);
        ...
        else if (type == "appsUpdate")
        {
            ...
            if (doc.containsKey("apps"))
            {
                for (auto app : doc["apps"].as<JsonArray>())
                {
                    ...
                    uint16_t duration = app.containsKey("duration") ? app["duration"].as<int>() : 0;
        ...
        {
            String city = doc.containsKey("city") ? doc["city"].as<String>() : String("");
            String apiKey = doc.containsKey("apiKey") ? doc["apiKey"].as<String>() : String("");
        ...
        else if (type == "setDisplayConfig")
        {
            ...
            String layout = doc.containsKey("layout") ? doc["layout"].as<String>() : String("");
            String colorCalibration = doc.containsKey("colorCalibration") ? doc["colorCalibration"].as<String>() : String("");

            // 这些字段当前固件未使用，但为了新协议对接，先保存到 settings（需要 saveSettings/loadSettings 支持）
            // 这里先做 ACK + broadcastConfig；具体持久化字段将在 settings 存储实现后生效
