...
void setup()
{
  ...
  LOG_INFO("[Main] === NeoClock 矩阵时钟启动 ===");

  // 初始化 LittleFS 文件系统
  LOG_INFO("[Main] 初始化文件系统...");
  if (!LittleFS.begin(true, "/littlefs"))
  {
    LOG_WARN("[Main] LittleFS 挂载失败！正在格式化...");
  }
  else
  {
  ...
  // 初始化音频采集管理器
  LOG_INFO("[Main] 初始化音频采集...");
  AudioManager.setup();

  // 初始化频谱分析管理器
  LOG_INFO("[Main] 初始化频谱分析...");
  SpectrumManager.setup();
  ...
  // 启动WebSocket服务器
  LOG_INFO("[Main] 启动WebSocket服务器...");
  ServerManager.setup(&webSocket);

  // 初始化 Liveview
  LOG_INFO("[Main] 初始化 Liveview...");
  ...
  {
    // AP 配网模式提示
    LOG_WARN("[Main] WiFi 未连接，进入配网模式");
    LOG_INFO("[Main] 请连接热点: %s", WebConfigManager.getAPName().c_str());
    LOG_INFO("[Main] 并访问 http://192.168.4.1 进行配网");
  }
  ...
  {
    LOG_INFO("[Main] 模式: AP 配网 (IP: %s)", WiFi.softAPIP().toString().c_str());
  }

  LOG_INFO("[Main] 应用数量: %d", Apps.size());
  LOG_INFO("[Main] Web控制面板: http://[Device IP]");
  LOG_INFO("[Main] WebSocket: ws://[Device IP]:81");
  LOG_INFO("========================================");
}
 ...
 *   2. AudioManager.tick() - 音频数据采集
 *   3. SpectrumManager.update() - FFT 频谱分析
 *   4. DisplayManager.tick() - 渲染当前帧到 leds[]
 *   5. Liveview.tick() - 采样 leds[] (纯内存操作，极快)
 *   6. PeripheryManager.tick() - 传感器读取、LDR 更新
 *   7. ServerManager.tick() - WebSocket 收包处理 (ws->loop())
 *   8. Liveview.flush() - 发送采样帧 (TCP 缓冲区最宽裕时)
 *
 * 性能优化：
...
void loop()
{
  ...
  // 音频采集
  AudioManager.tick();
  if (AudioManager.hasNewData())
  {
    SpectrumManager.update(AudioManager.getAudioBuffer(), FFT_SIZE);
    AudioManager.resetNewDataFlag();
  }
